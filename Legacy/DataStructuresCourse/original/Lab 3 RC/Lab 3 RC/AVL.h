...
AVL::AVL(vector<string> _data)
{
	size = _data.size();
	SCREEN = new char *[8];
	for (int i = 0; i < 8; i++) SCREEN[i] = new char[80];
	vector<int> temp;
	for each (string item in _data)
...
///<summary>Получение элементов дерева</summary>
vector<string> AVL::GetDataStr()
{
	vector<string> result;
	//for each (int item in data)
	//{
	//	result.push_back(stoi(&item, nullptr, 10));
	//}
	return result;
}
///<summary>Получение элементов дерева</summary>
vector<int> AVL::GetData()
{
	return data;
}
#pragma endregion
