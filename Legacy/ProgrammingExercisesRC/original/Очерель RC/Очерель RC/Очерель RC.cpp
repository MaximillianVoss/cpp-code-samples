...
int main() {
	...
	queue queue; //Создаём экземпляр класса "Очередь"
	queue.reset(); //Очищаем очередь
	int i = 0;
	while (!queue.full()) //Пока очередь не полна
		queue.enqueue(str[i++]);//Заполняем очередь элементом
	while (!queue.empty()) { //Пока очередь не пуста
		cout << queue.dequeue() << endl; //выводим головной элемент из очереди
	}
	system("pause");
}
