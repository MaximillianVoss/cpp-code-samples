...
#pragma region GET
	...
	/// <returns></returns>
	T Get(int i) const {
		if (i >= 0 && static_cast<size_t>(i) < this->items.size())
			return this->items[static_cast<size_t>(i)];
		return T{};
	}
	/// <summary>
	///
	...
	vector<Pair<double, double>> GetValues(double from, double to, double step) {
		...
		for (double i = from; i < to; i += step) {
			double value = 0.0;
			for (auto coefficient = items.rbegin(); coefficient != items.rend(); ++coefficient)
				value = value * i + static_cast<double>(*coefficient);
			Pair<double, double> pair = Pair<double, double>(i, value);
...
#pragma region SET
	...
	void Set(int i, T value) {
		if (i >= 0 && static_cast<size_t>(i) < items.size())
			this->items[static_cast<size_t>(i)] = value;
	}
#pragma endregion
#pragma region Other
	void Down(size_t n) {
...
#pragma region Operators
	...
	template <typename T>
	friend ostream& operator << (ostream& out, const Polynom<T>& a) {
		bool wroteTerm = false;
		for (size_t offset = a.items.size(); offset > 0; --offset) {
			const size_t i = offset - 1;
			if (a.items[i] != 0) {
				if (wroteTerm && a.items[i] > 0)
					out << '+';
				out << a.items[i];
				if (i > 0)
					out << "*x^" << i;
