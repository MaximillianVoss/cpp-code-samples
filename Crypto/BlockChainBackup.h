...
#pragma region Поля
	...
	/// </summary>
	size_t counterVotingFirstStage = 0;
	/// <summary>
	/// Общее количество голосований
	/// </summary>
	size_t counterVotingTotal = 0;
	/// <summary>
	/// счетчик успешных голосований
	/// </summary>
	size_t counterVotingCompleted = 0;
	/// <summary>
	/// счетчик неудачных голосаний
	/// </summary>
	size_t counterVotingFailed = 0;
	/// <summary>
...
#pragma region Генерация блока
	...
	Block* GenerateBlock(bool onlyOneRound = false) {
		Block* block = new Block(SHA1().GetHash(to_string(this->counterBlocks + 1)), "");
		this->counterVotingFirstStage++;
		Voting(block, {}, onlyOneRound);
		if (this->Add(block))
			return block;
		else
			return NULL;
