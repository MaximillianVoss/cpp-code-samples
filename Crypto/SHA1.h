...
private:
	...
	void update(istream& is) {
		while (true)
		{
			char* sbuf = new char[BLOCK_BYTES];
	...
	void reset(uint32_t digest[], string& buffer, uint64_t& transforms)
	{
	...
	uint32_t blk(const uint32_t* block, const size_t i)
	{
	...
	void buffer_to_block(const string& buffer, uint32_t* block)
	{
